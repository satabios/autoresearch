...
def compute_worker_allocation(
    ...
    Args:
        ...
        workers_per_gpu: optional per-GPU cap. ``1`` means one worker per GPU.
    ...
    for gpu in gpus:
        ...
        workers_here = min(workers_here, _MAX_RAY_WORKERS_PER_GPU)
        if workers_per_gpu is not None:
            workers_here = min(workers_here, workers_per_gpu)
        if workers_here <= 0:
            continue
        allocations.append(
            WorkerAllocation(
    ...
    for group_idx in range(num_groups):
        ...
        allocations.append(
            WorkerAllocation(
                ...
                vram_per_worker_gb=model_vram_gb,
                reserved_safety_gb=safety_net_gb,
                shard_device_ids=tuple(g.device_id for g in group_gpus),
            )
        )
