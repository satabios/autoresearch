...
class DynamicScheduler:
    ...
    def poll_and_rebalance(self) -> dict:
        ...
        # Resolve and clear accumulated steal_refs to avoid unbounded growth
        if self._steal_refs:
            try:
                ray.get(self._steal_refs)
            except Exception as e:
                _log.warning("steal_layers result collection failed: %s", e)
            self._steal_refs.clear()
