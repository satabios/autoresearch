...
def _get_qscheme_types() -> dict:
    ...
    try:
        from aimet_onnx.common.defs import qtype  # type: ignore[import-untyped]
        _QSCHEME_TYPES = {
            "w8a8": (qtype.int(8), qtype.int(8)),
            "w8a16": (qtype.int(8), qtype.int(16)),
            "w4a8": (qtype.int(4), qtype.int(8)),
            "w4a16": (qtype.int(4), qtype.int(16)),
...
class OnnxQuantSimAdapter:
    ...
    @property
    def sim(self) -> Any:
        """The underlying aimet_onnx QuantizationSimModel."""
        return self._sim
