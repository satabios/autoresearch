...
from octopus.capabilities import (
    ...
    validate_sharding_configuration,
)
...
from octopus.profiler import pick_profiling_gpu, profile_model_vram, profile_ort_vram
...
class Octopus:
    ...
    def __init__(
        ...
        Args:
            ...
            workers_per_gpu: Cap replicas per GPU. Use ``1`` for one worker per GPU.
    ...
    def _ensure_initialized(self, sample_batch: Any) -> None:
        ...
        self._profile = self._profile_vram(sample_batch, profiling_gpu)
        ...
        if not ray.is_initialized():
            init_ray(address=self._ray_address, log_to_driver=False)
    ...
    def _profile_vram(self, sample_batch: Any, profiling_gpu: int) -> VRAMProfile:
        """Profile model VRAM with backend-aware strategy."""
        if self._adapter.model_type_name in {"onnx", "onnx_quantsim"}:
            from octopus.adapters.onnx_rt import extract_model_bundle_to_tempdir

            bundle_dir: Optional[tempfile.TemporaryDirectory[str]] = None
            try:
                bundle_dir, onnx_path = extract_model_bundle_to_tempdir(
                    self._adapter.state_bytes()
                )
                return profile_ort_vram(
                    onnx_path=onnx_path,
                    sample_feed_fn=lambda: self._normalize_ort_batch(sample_batch),
        ...
        return profile_model_vram(
            self._adapter,
            sample_batch,
            device_id=profiling_gpu,
        )

    @staticmethod
    def _normalize_ort_batch(batch: Any) -> dict[str, Any]:
        """Normalize one ONNX batch into a feed dict of numpy arrays."""
        if isinstance(batch, dict):
            return {
                str(k): v.detach().cpu().numpy() if isinstance(v, torch.Tensor) else v
    ...
    def sensitivity_scan(
        ...
        # Wait for static chunks to finish (return values also available via get_results)
        ray.get(process_refs)

        # Stop dynamic scheduler and assign any unprocessed layers
        if scheduler is not None:
            ...
            if remaining:
                ...
                ray.get(workers[0].process_layers.remote(remaining))

        # Collect all results (static chunks + steal_layers) from each worker
        all_results: dict[str, float] = {}
        result_refs = [w.get_results.remote() for w in workers]
        for worker_results in ray.get(result_refs):
    ...
    def backend_capabilities(self) -> dict[str, Any]:
        """Capabilities for backend selected by this Octopus instance."""
        return get_backend_capabilities(self._adapter.model_type_name)

    @staticmethod
    def get_backend_capabilities(model_type_name: str) -> dict[str, Any]:
        """Static helper to query backend capability policy."""
        return get_backend_capabilities(model_type_name)
