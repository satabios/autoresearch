...
class WorkerPool:
    ...
    def _start_standard(self) -> None:
        ...
        for alloc in self._plan.allocations:
            ...
            for _ in range(alloc.num_workers):
                option_kwargs: dict[str, Any] = {"num_gpus": frac_gpu}
                if env_vars:
                    option_kwargs["runtime_env"] = {"env_vars": env_vars}
                actor = InferenceWorker.options(**option_kwargs).remote(
                    self._model_bytes,
    ...
    def _start_sharded(self) -> None:
        ...
        for group_rank, alloc in enumerate(self._plan.allocations):
            device_ids = list(alloc.shard_device_ids) or [alloc.device_id]
