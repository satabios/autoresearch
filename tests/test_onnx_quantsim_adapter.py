...
class TestFromStateBytes:
    def _make_adapter_from_tarball(self, tarball_bytes, eval_fn=None):
        ...
        with patch("octopus.adapters.onnx_quantsim._QSCHEME_TYPES", None), \
            ...
            sys.modules["aimet_onnx.common.defs"].QuantScheme.post_training_tf = "tf"
            qtype = sys.modules["aimet_onnx.common.defs"].qtype
            qtype.int.side_effect = lambda bits: f"int{bits}"
            qtype.from_string.side_effect = lambda name: name
            sys.modules["aimet_onnx.quantsim"].load_encodings_to_sim = MagicMock()
    ...
    def test_from_state_bytes_calls_load_encodings_strict_false(self):
        ...
        with patch.dict("sys.modules", {
            ...
            load_enc = MagicMock()
            sys.modules["aimet_onnx.quantsim"].load_encodings_to_sim = load_enc
