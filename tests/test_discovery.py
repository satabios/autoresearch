...
class TestRecommendStrategy:
    ...
    def test_model_fits_single_gpu_uses_replicas(self, mock_gpu_2x80):
        """When model fits on one GPU, auto should pick replica workers."""
        caps = {
            "shared_model_multi_gpu": True,
            "shared_model_sharding_strategies": ("pp", "tp"),
        }
        plan = recommend_strategy(
            mock_gpu_2x80, model_vram_gb=10.0, safety_net_gb=1.0,
            backend_capabilities=caps,
        )
        assert plan.sharding_required is False
        assert plan.sharding_strategy is None
        assert plan.total_workers >= 2

    def test_model_too_large_falls_back_to_pp(self, mock_gpu_2x80):
        """When model doesn't fit on one GPU, auto should shard with PP."""
        caps = {
            "shared_model_multi_gpu": True,
            "shared_model_sharding_strategies": ("pp", "tp"),
        }
        plan = recommend_strategy(
            mock_gpu_2x80, model_vram_gb=100.0, safety_net_gb=1.0,
            backend_capabilities=caps,
        )
        assert plan.sharding_required is True
        assert plan.sharding_strategy == "pp"

    def test_model_too_large_onnx_backend_uses_pp(self, mock_gpu_4x24):
        """ONNX backend only supports PP — auto should use it."""
        caps = {
            "shared_model_multi_gpu": True,
            "shared_model_sharding_strategies": ("pp",),
        }
        plan = recommend_strategy(
            mock_gpu_4x24, model_vram_gb=60.0, safety_net_gb=1.0,
            backend_capabilities=caps,
        )
        assert plan.sharding_required is True
        assert plan.sharding_strategy == "pp"

    def test_no_sharding_support_raises(self, mock_gpu_1x8):
        """Backend with no sharding should raise when model doesn't fit."""
        caps = {
            "shared_model_multi_gpu": False,
            "shared_model_sharding_strategies": (),
        }
        with pytest.raises(InsufficientVRAMError, match="Backend does not support sharding"):
            recommend_strategy(
                mock_gpu_1x8, model_vram_gb=10.0, safety_net_gb=1.0,
                backend_capabilities=caps,
            )

    def test_no_capabilities_provided_raises_when_model_too_large(self, mock_gpu_1x8):
        """None capabilities should raise when model doesn't fit."""
        with pytest.raises(InsufficientVRAMError):
            recommend_strategy(
                mock_gpu_1x8, model_vram_gb=10.0, safety_net_gb=1.0,
    ...
    def test_respects_workers_per_gpu(self, mock_gpu_2x80):
        """workers_per_gpu should be forwarded to replica allocation."""
        caps = {
            "shared_model_multi_gpu": True,
            "shared_model_sharding_strategies": ("pp",),
        }
        plan = recommend_strategy(
            mock_gpu_2x80, model_vram_gb=10.0, safety_net_gb=1.0,
            backend_capabilities=caps, workers_per_gpu=1,
        )
        assert plan.total_workers == 2
        assert all(a.num_workers == 1 for a in plan.allocations)

    def test_respects_max_workers(self, mock_gpu_2x80):
        """max_workers cap should be forwarded."""
        caps = {
            "shared_model_multi_gpu": True,
            "shared_model_sharding_strategies": ("pp",),
        }
        plan = recommend_strategy(
            mock_gpu_2x80, model_vram_gb=10.0, safety_net_gb=1.0,
            backend_capabilities=caps, max_workers=3,
        )
        assert plan.total_workers == 3

    def test_exact_fit_uses_replicas(self):
        """Model that exactly fills one GPU should use replicas, not sharding."""
        gpus = [GPUInfo(device_id=0, name="G", total_vram_gb=16.0,
                        available_vram_gb=11.0, compute_capability=(8, 0))]
        caps = {
            "shared_model_multi_gpu": True,
        ...
        plan = recommend_strategy(
            gpus, model_vram_gb=10.0, safety_net_gb=1.0,
            backend_capabilities=caps,
        )
        assert plan.sharding_required is False
        assert plan.total_workers == 1
